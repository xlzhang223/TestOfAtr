...
inline mirror::Object* Heap::AllocObjectWithAllocator(Thread* self,
  ...
  if (kCheckLargeObject && UNLIKELY(ShouldAllocLargeObject(klass, byte_count))) {
    ...
    if (obj != nullptr) {
      ...
      // if(byte_count >= (unsigned long)leakleak::getInstance()->getTsize())
      //if(Thread::Current()->Getiscopy()==0)
        leakleak::getInstance()->new_obj(obj.Ptr(),byte_count);
      ...
      if (obj == nullptr) {
        ...
        if (!self->IsExceptionPending()) {
          ...
          auto ret = AllocObject</*kInstrumented*/true>(self,
                                                    ...
                                                    pre_fence_visitor);
          //if(Thread::Current()->Getiscopy()==0)
            leakleak::getInstance()->new_obj(ret,byte_count);
          return ret;
          //end
