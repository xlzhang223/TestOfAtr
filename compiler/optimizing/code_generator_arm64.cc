...
void CodeGeneratorARM64::genaccessbit(Register obj,int obj_size = (1<<20)){
  ...
  uint64_t temp1=2584,temp2=2592;//,temp3=2456;//,temp4=2464;//,temp5=2472;
...
//   __ B(&lable_zhang); //ingnore access bit  debug!! 
    if(obj_size == (1<<20)){
      ...
      __ Cmp(r2,obj.X());
      __ B(eq, &lable_zhang);
      __ Str(obj.X(),MemOperand(Tr,temp2));
    }
    else{
    ...
    __ Mov(r3,leakleak::getInstance()->get_main_begin());
    __ Cmp(obj.W(),r3);//"compare with int"
    __ B(le, &lable_zhang);

    __ Mov(r2,leakleak::getInstance()->get_main_end());
    __ Cmp(obj.W(),r2);//"compare with int"
    __ B(ge, &lable_zhang);
  ...
  // __ Add(sp,sp,16);
    ...
    LOG(WARNING)<<"Leakleak,genaccessbit error";
...
void CodeGeneratorARM64::GenerateFrameEntry() {
  ...
  //zhang bit access
  //leakleak::log_app_name();
  genaccessbit(vixl::aarch64::w1,1024);
  //end
